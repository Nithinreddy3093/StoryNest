...
export const StoryReaderPage: React.FC<StoryReaderPageProps> = ({
  ...
  const seriesInfo = story?.seriesId ? getSeriesById(story.seriesId) : null;
  const seriesStories = story?.seriesId ? getStoriesForSeries(story.seriesId) : [];
  const currentPart = story?.seriesPart || 1;
  const nextStoryInSeries = seriesStories.find((s) => (s.seriesPart || 0) > currentPart && s.id !== story?.id);
            ...
            <div className={`p-4 sm:p-5 rounded-2xl border shadow-xl ${getContainerBg()}`}>
              <div className="flex sm:block gap-4 items-center sm:items-start mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Series Saga Banner if in a series */}
                  {seriesInfo && (
                    <button
                      onClick={() => navigate(`series:${seriesInfo.id}`)}
                      className="mt-1.5 w-full text-left p-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[11px] font-semibold flex items-center gap-1.5 hover:bg-amber-500/20 transition-colors"
                    >
                      <Layers className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                      <span className="truncate">
                        Part {story.seriesPart || 1} of {seriesInfo.title}
                      </span>
                    </button>
                  )}
            ...
            <div className={`p-5 sm:p-10 rounded-2xl border shadow-2xl ${getContainerBg()}`}>
              {currentChapter ? (
                <>
                  ...
                  {/* Next in Series Promotion when at the end of the book */}
                  {currentChapterIndex === chapters.length - 1 && nextStoryInSeries && (
                    <div className="p-4 rounded-xl bg-gradient-to-r from-amber-500/20 via-amber-500/10 to-transparent border border-amber-500/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-left">
                      <div className="space-y-0.5">
                        <span className="text-[10px] uppercase font-black tracking-widest text-amber-400 flex items-center gap-1">
                          <Layers className="w-3 h-3" /> Next Volume in {seriesInfo?.title}
                        </span>
                        <h4 className="font-serif-heading text-sm font-bold text-slate-100">
                          Part {nextStoryInSeries.seriesPart}: {nextStoryInSeries.title}
                        </h4>
                      </div>
                      <button
                        onClick={() => {
                          navigate(`read:${nextStoryInSeries.id}`);
                          window.scrollTo({ top: 0, behavior: 'smooth' });
                        }}
                        className="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs flex items-center gap-1.5 transition-all shadow-md shrink-0"
                      >
                        <span>Start Reading Part {nextStoryInSeries.seriesPart}</span>
